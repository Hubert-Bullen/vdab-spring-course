...
public class BlogRepositoryTest {
    ...
    Blog cookie = new Blog("CookieMonster, Sesame Street's Serial Killer?", new Date());
    ...
    @Test
    public void testCanIAddBlog(){
        br.save(cookie);
        assertEquals("CookieMonster, Sesame Street's Serial Killer?",br.findOne(1).getTitle());
    }

    @Test
    public void testAuthorForBlog(){
        cookie.setAuthor(jim);
        br.save(cookie);
        assertEquals("Jim69",br.findOne(1).getAuthor().getUsername());

    }


    // TODO More tests, also for BlogPost and Comments.
